const DOM_id_class_variables = {
	...
	mapPopOutClass: 'map-pop-out-icon',
	selectedBtnClass: 'selected-btn',
	view_btn_containerClass: 'view-btn-container',
	mode_btns_containerID: 'mode-btns',
	projection_containerDiv: 'projection-container',
	projectionMode_mainView: 'projection-main-view',
	projection_label_element: 'label-element',
	projection_btn_label: 'btn-label',
	projection_cascade_btn_label: 'cascade-label',
	projection_btn_arrow: 'label-arrow',
	projection_statistics: 'statistics-container',
	component_container: 'component-container',
	zoom_component_container: 'zoom-container',
...
const buildEcosystemProjectionView = (hashParams) => {
	...
	ecoSystem2050ProjectionView.append(projectionStatisticsFooter);
...
const createProjectionStatisticsHTML = () => {
	const projectionStatisticsContainer = document.createElement('div');

	projectionStatisticsContainer.id =
		DOM_id_class_variables['projection_statistics'];
	const statisticsHTML = `<div id='statistics'></div>`;
	projectionStatisticsContainer.innerHTML = statisticsHTML;

	return projectionStatisticsContainer;
};

const buildExplorerMode = ({
	explorerViewComponents,
	wte_categories,
	hashParams,
}) => {
	const viewContainer = buildExplorerModeHTML(
		explorerViewComponents,
		wte_categories,
		hashParams,
	);
...
const createMobileBackgroundElement = (wte_categories) => {
	const backgroundDiv = document.createElement('div');
	backgroundDiv.id = DOM_id_class_variables['mobile_UI'];
	backgroundDiv.classList.add(`${DOM_id_class_variables['hiddenClass']}`);
	const labelContainer = document.createElement('div');
	labelContainer.id = `${DOM_id_class_variables['explore_mobile_component']}`;

	console.log(wte_categories);
	const mobileLabelsAndDropdown = wte_categories.map((category) => {
		category.elementID = category.elementID + '-mobile';
		const labelElement = createComponentLabel({ category });
		// const mobileDropdown = createDropdownComponent({ category });

		labelElement.lastElementChild.remove();
		// labelElement.append(mobileDropdown);
		labelContainer.append(labelElement);
	});
	backgroundDiv.append(labelContainer);

	return backgroundDiv;
};

const buildExplorerModeHTML = (
	explorerViewComponents,
	wte_categories,
	hashParams,
) => {
	...
	const mobileBackgroundDiv = createMobileBackgroundElement(wte_categories);
...
const createMainView = (idString) => {
	...
	zoomContainer.classList.add(
		`${DOM_id_class_variables['zoom_component_container']}`,
	);
	zoomContainer.append(zoomComponent);

	mainViewDiv.append(zoomContainer);
...
const createViewButtonsUI = (
	...
	const modeButtonsHTML = `
                            <button value='explore' class="viewMode btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.mode === 'explore' || hashParams.mode === '' ? selectedBtnClass : ''}">
                              <calcite-icon id='explore-mode-tooltip' icon="information" scale="s"></calcite-icon>
                                ...
                                <span>${getString('modeButton__exploreCurrent__tooltip')}</span>
                              </calcite-tooltip>
                              <span>${getString('modeButton__exploreCurrent')}</span>
                            </button>
                            <button value='change' class="viewMode btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.mode === 'change' ? selectedBtnClass : ''}">
                              <calcite-icon id='projection-mode-tooltip' icon="information" scale="s"></calcite-icon>
                                ...
                                <span>${getString('modeButton__projection__tooltip')}</span>
                              </calcite-tooltip>
                              <span>${getString('modeButton__projection')}</span>
                            </button>
                            <span class="${DOM_id_class_variables['projection_label_element']} btn-label" > ${getString('modeButton__label')} </span>
                            <div class="${DOM_id_class_variables['projection_label_element']} label-arrow ${hashParams?.mode !== 'change' ? DOM_id_class_variables['hiddenClass'] : ''}">
                            <calcite-icon icon="arrow-bold-down" scale="s" style="color:#ffffff;"/>
                            </div>
                            `;
	...
	const projectionChangeModelButtonsHTML = `
                            <button value="sustainable" class="changeModel btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.model === 'sustainable' ? selectedBtnClass : ''}">
                              <calcite-icon id='sustainable-model-tooltip' icon="information" scale="s"></calcite-icon>
                                ...
                                <span>${getString('projectionTypeButton__sustainableModel__tooltip')}</span>
                              </calcite-tooltip>
                              <span>${getString('projectionTypeButton__sustainableModel')}</span>
                            </button>
                            <button value="self-reliant" class="changeModel btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.model === 'self-reliant' ? selectedBtnClass : ''}">
                              <calcite-icon id='high-emissions-tooltip' icon="information" scale="s"></calcite-icon>
                                ...
                                <span>${getString('projectionTypeButton__regionalRivalsModel__tooltip')}</span>
                              </calcite-tooltip>
                              <span>${getString('projectionTypeButton__regionalRivalsModel')}</span>
                            </button>
                            <span class="cascade-label  btn-label" > ${getString('projectionTypeButton__label')} </span>
                            <div class="${DOM_id_class_variables['projection_btn_arrow']} ${!hashParams.changeType ? DOM_id_class_variables['hiddenClass'] : ''}">
                            <calcite-icon icon="arrow-bold-down" scale="s" style="color:#ffffff;"/>
                            </div>
                            `;
	...
	const filterChangeTypeBtnsHTML = `
                          <button value='aridity' category='aridity' class='filterChangeType btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.changeType === 'aridity' ? selectedBtnClass : ''}'> 
                            ...
                            <span>${getString('projectionFilterButton__aridity__tooltip')}</span>
                            </calcite-tooltip>
                            <span>${getString('projectionFilterButton__aridity')}</span> 
                          </button>
                          <button value='temperature' category='temperature' class='filterChangeType btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.changeType === 'temperature' ? selectedBtnClass : ''}'> 
                            ...
                            <span>${getString('projectionFilterButton__temperature__tooltip')}</span>
                            </calcite-tooltip>
                            <span>${getString('projectionFilterButton__temperature')}</span>
                          </button>
                          <button value='landCover' category='landCover' class='filterChangeType btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.changeType === 'landCover' ? selectedBtnClass : ''}'> 
                            ...
                            <span>${getString('projectionFilterButton__landCover__tooltip')}</span>
                            </calcite-tooltip>
                            <span>${getString('projectionFilterButton__landCover')}</span>
                          </button>
                          <button value='total__changes' category='total__changes' class='filterChangeType btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.changeType === 'total__changes' ? selectedBtnClass : ''}'> 
                            ...
                            <span>${getString('projectionFilterButton__anyChange__tooltip')}</span>
                            </calcite-tooltip>
                            <span>${getString('projectionFilterButton__anyChange')}</span>
                           </button>
                          <button value='all__changes' category ='all__changes' class='filterChangeType btn ${DOM_id_class_variables['UI_btnsClass']} ${hashParams.changeType === 'all__changes' ? selectedBtnClass : ''}'>
                            ...
                            <span>${getString('projectionFilterButton__allChanges__tooltip')}</span>
                            </calcite-tooltip>
                            <span>${getString('projectionFilterButton__allChanges')}</span>
                          </button>
                          <span class="cascade-label btn-label" > ${getString('projectionFilterButton__label')} </span>
                          `;

	buttonsContainer.innerHTML = modeButtonsHTML;
	buttonsContainer.id = `${DOM_id_class_variables['mode_btns_containerID']}`;
...
const createSupplementalViewHTMLs = ({
	...
	const bottomLabel = createComponentLabel({ category });

	const viewDiv = document.createElement('arcgis-map');
	viewDiv.id = category.elementID;
...
const createDropdownComponent = ({ category, getString }) => {
	console.log(category);
	const dropdownFeatures = category.features;
	console.log(dropdownFeatures);
	const dropdownSelections = dropdownFeatures
		.map((featureEntry, index) => {
			const selection = `
      <li class="choice" data-id="${featureEntry['id']}" value="${featureEntry['wte']}">${getString(featureEntry['wte'].trim())}
    ...
    <span class="${DOM_id_class_variables[`dropDownDisplayClass`]}"></span>
	  <span class="selected-tooltip"></span>
	...
	dropdownContainer.id = `${category.elementID}-dropdown`;
	dropdownContainer.classList.add(DOM_id_class_variables['dropdownElement']);
	dropdownContainer.innerHTML = dropdownHTML;

	return dropdownContainer;
};

const createComponentLabel = ({ category }) => {
	const infoIcon = `
  
    <calcite-icon id='${category.elementID}-tooltip' icon="information" scale="s"></calcite-icon>
  
  <calcite-tooltip reference-element="${category.elementID}-tooltip">
    <span>${category.tooltip}</span>
  </calcite-tooltip>
  `;
	const labelString = `<span class="pop-out-title">${category.title}</span>`;
...
const createUserIconElement = (getCredentialsCallback) => {
	const userAccountContainer = document.createElement('div');
	userAccountContainer.id = `${DOM_id_class_variables['user_icon_element']}`;
  ...
  `;
	...
	document.querySelector('main').append(userAccountContainer);
};
...
const popOutMapComponent = () => {
	...
	const popOutMapHTML = `
                          ...
                          <div class="label">
                            <div>
                                  <span class='pop-out-info'>                                    
                                    <calcite-icon id='minimap-tooltip' icon="information" scale="s"></calcite-icon>
                                    <calcite-tooltip reference-element="minimap-tooltip">
                                      <span class='tooltipText'>What a helpful tooltip for the minimap</span>
                                    </calcite-tooltip>
                                  </span>
                                  <span class="pop-out-title"></span>
                            </div>
                                   <div><calcite-icon icon="x" /></div>
                                   </div>
                          </div>
                        `;
...
const nonValidCombinationSelection = () => {
	...
	//modified error-notification copy for demo-purposes.
